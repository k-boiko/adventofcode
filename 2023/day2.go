...
func Day2(part string) {
	...
	for _, line := range strings.Split(string(d2_test), "\n") {
		...
		for _, set := range strings.Split(match[2], "; ") {
			for _, cubes := range strings.Split(set, ", ") {
				cubesInfo := strings.Split(cubes, " ")
				cubesAmount, _ := strconv.Atoi(cubesInfo[0])
				if part == "1" && cubesAmount > colorsMaxValue[cubesInfo[1]] {
					isImpossible = true
					break
				}
				if part == "2" && cubesAmount > colorsMinValue[cubesInfo[1]] {
					colorsMinValue[cubesInfo[1]] = cubesAmount
				}
			}
			if isImpossible {
				break
		...
		pow := 1
		for _, num := range colorsMinValue {
			pow *= num
		}
		powsSum += pow
...
const d2_test = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`

const d2_prod = `Game 1: 7 blue, 9 red, 1 green; 8 green; 10 green, 5 blue, 3 red; 11 blue, 5 red, 1 green
