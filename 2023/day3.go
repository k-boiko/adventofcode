...
func Day3(part string) {
	adjacentNums := map[string]int{}
	lines := strings.Split(string(d3_prod), "\n")
	for row, line := range lines {
		for col, char := range line {
			if part == "1" && (char == '.' || IsNumericChar(byte(char))) {
				continue
			}
			...
			maybePushAdjacentNum(lines, row-1, col+1, &toAdd)
			maybePushAdjacentNum(lines, row-1, col, &toAdd)
			maybePushAdjacentNum(lines, row-1, col-1, &toAdd)
			maybePushAdjacentNum(lines, row, col-1, &toAdd)
			maybePushAdjacentNum(lines, row, col+1, &toAdd)
			maybePushAdjacentNum(lines, row+1, col-1, &toAdd)
			maybePushAdjacentNum(lines, row+1, col, &toAdd)
			maybePushAdjacentNum(lines, row+1, col+1, &toAdd)
...
func maybePushAdjacentNum(lines []string, y int, x int, toAdd *map[string]int) {
	if IsNumericChar(lines[y][x]) {
		adjacentNumCoord, adjacentNum := getFullNumBfs(y, x, lines)
		(*toAdd)[adjacentNumCoord] = adjacentNum
	}
}

func getFullNumBfs(y int, x int, lines []string) (string, int) {
	num := string(lines[y][x])
	start := x
	end := x
	for i := x - 1; i >= x-2; i-- {
		if i < 0 || !IsNumericChar(lines[y][i]) {
			break
		}
		num = string(lines[y][i]) + num
		start = i
	}

	for i := x + 1; i <= x+2; i++ {
		if i > len(lines[y]) || !IsNumericChar(lines[y][i]) {
			break
		}
		num = num + string(lines[y][i])
		end = i
	}
	res, _ := strconv.Atoi(num)

	return fmt.Sprint(y, ".", start, "-", end), res
}

const d3_test = `467..114..
...*......
..35..633.
......#...
617*......
