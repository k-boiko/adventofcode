...
func Day1(part string) {
	...
	for _, line := range strings.Split(string(d1_prod), "\n") {
		...
		for _, char := range line {
			if char >= '0' && char <= '9' {
				lineNumStr += string(char)
			}
			if part == "2" {
				state += string(char)
				for _, replacer := range replacers {
					if strings.HasSuffix(state, replacer.word) {
						lineNumStr += replacer.num
					}
				}
			}
		}
		lineNumStr = string(lineNumStr[0]) + string(lineNumStr[len(lineNumStr)-1])
		lineNum, err := strconv.Atoi(lineNumStr)
		if err != nil {
			log.Fatalf("cannot parse str: %v", lineNumStr)
		}
		sum += lineNum
	}
